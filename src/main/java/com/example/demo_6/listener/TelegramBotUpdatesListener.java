package com.example.demo_6.listener;

import com.example.demo_6.keyboard.KeyBoardShelter;
...
import com.example.demo_6.repository.PersonCatRepository;
import com.example.demo_6.repository.PersonDogRepository;
import com.example.demo_6.repository.ReportDataRepository;
import com.example.demo_6.service.ReportDataService;
        ...
        if (matcher.matches()) {
            ...
            try {
                ...
                reportDataService.uploadReportData(update.message().chat().id(), fileContent, file,
                        ration, health, habits, fullPathPhoto, (java.sql.Date) dateSendMessage, timeDate, daysOfReports);
            ...
            try {
                ...
                reportDataService.uploadReportData(update.message().chat().id(), fileContent, file, update.message().caption(),
                        fullPathPhoto, (java.sql.Date) dateSendMessage, timeDate, daysOfReports);
                telegramBot.execute(new SendMessage(update.message().chat().id(), "Отчет успешно принят!"));
                System.out.println("Отчет успешно принят от: " + update.message().chat().id());
            } catch (IOException e) {
                System.out.println("Ошибка загрузки фото!");
